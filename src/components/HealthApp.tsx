...
export function HealthApp({ initialData, demo, onLogout }: { initialData: HealthData; demo: boolean; onLogout: () => void }) {
  ...
  useEffect(() => {
    ...
    const timer = window.setInterval(() => {
      ...
      if (due) new Notification('Future Health medication reminder', { body: `${due.medication.name} ${due.medication.dosage} ${due.medication.unit} is due now.` })
      ...
      if (appointment) new Notification('Future Health appointment reminder', { body: `${appointment.appointmentType} with ${appointment.doctor} is ${formatDateTime(appointment.startsAt)}.` })
    }, 60_000)
  ...
  return (
    <div className="health-shell">
      <aside className={`side-nav ${mobileOpen ? 'open' : ''}`}>
        <div className="brand"><div className="brand-mark"><HeartPulse /></div><div><strong>FUTURE</strong><span>HEALTH</span></div></div>
        <button className="mobile-close" onClick={() => setMobileOpen(false)} aria-label="Close menu"><X /></button>
        <div className="profile-mini"><div className="avatar">{initials(data.profile.fullName)}</div><div><strong>{data.profile.fullName || 'Health Member'}</strong><span>{demo ? 'Fictional demo profile' : 'Private health space'}</span></div></div>
...
function HealthView({ data, add }: any) {
  ...
  return <><PageHeading eyebrow="MEASUREMENTS" title="Health metrics" description="Record observations without turning numbers into diagnoses." action="Add measurement" onAction={() => add('metric')} />
    ...
    <section className="panel caution-card"><Stethoscope /><div><h3>Measurements are context, not conclusions.</h3><p>Future Health does not diagnose. Consider discussing unusual or persistent readings with a qualified healthcare professional.</p></div></section>
